...
export function QuestioningToolPreviewModal({
  questions,
  onClose,
}: QuestioningToolPreviewModalProps) {
  const knowledgeQuestions = questions.filter(
    (q) => q.testType === "Knowledge",
  );
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-7xl max-h-[95vh] flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-[#E0E0E0] flex justify-between items-center flex-shrink-0">
          <div>
            <h2 className="text-lg font-semibold text-[#333]">
              Questioning Tool Preview & Export
            </h2>
            <p className="text-sm text-[#666]">
              Oral Interview Script - Shielded Metal Arc Welding (SMAW) NC II
            </p>
          </div>
          <button
            aria-label="Close"
            className="p-2 hover:bg-[#F5F5F5] rounded transition-colors text-gray-600"
            onClick={onClose}
          >
        ...
        <div className="px-6 py-3 bg-[#E8F5E9] border-b border-[#A5D6A7] flex-shrink-0">
          <div className="flex items-start gap-2">
            <span className="text-lg">
              <CheckBadgeIcon className="w-8 h-8 inline text-green-700" />
            </span>
            <div className="flex-1">
              <div className="text-sm font-medium text-[#2E7D32]">
                {questions.length} questions ready for oral assessment • Binary
                Scoring (S/NS)
              </div>
              <div className="text-xs text-[#2E7D32] mt-1">
                {knowledgeQuestions.length} Knowledge-based •{" "}
                {scenarioQuestions.length} Scenario-based
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto bg-[#F5F5F5] p-6">
          <div className="bg-white border border-[#E0E0E0] rounded-lg shadow-sm max-w-[800px] mx-auto p-8">
            ...
            <div className="text-center mb-6 pb-6 border-b-2 border-[#1976D2]">
              ...
              <div className="text-sm text-[#666]">
                ...
                <div className="text-xs">
                  Binary Assessment: Satisfactory (S) / Not Satisfactory (NS)
                </div>
              </div>
            </div>
            ...
            <div className="mb-6 p-4 bg-[#E3F2FD] border border-[#90CAF9] rounded">
              <div className="text-sm font-semibold text-[#1976D2] mb-2">
              ...
              <ul className="text-xs text-[#1565C0] space-y-1 ml-4">
                ...
                <li>
                  • Record responses using S (Satisfactory) or NS (Not
                  Satisfactory)
                </li>
                <li>• Any unsafe knowledge response = Not Satisfactory</li>
                <li>
                  • Use the model answers as a guide for acceptable responses
                </li>
              </ul>
            </div>
            ...
            <div className="space-y-4">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-[#666] mb-2">
                        <strong>Dimension:</strong> {q.dimension} |{" "}
                        <strong>Type:</strong> {q.testType}-based
                      </div>
                      ...
                      <div className="mt-2 flex items-center gap-4 text-xs">
                        <div className="flex items-center gap-1">
                          <span className="w-3 h-3 border-2 border-[#2E7D32] rounded-full" />
                          <span className="text-[#2E7D32] font-medium">S</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <span className="w-3 h-3 border-2 border-[#C62828] rounded-full" />
              ...
              {questions.length > 5 && (
                <div className="p-3 bg-[#F5F5F5] rounded text-center text-xs text-[#666] italic">
                  ... {questions.length - 5} more questions (Total:{" "}
        ...
        <div className="px-6 py-4 border-t border-[#E0E0E0] bg-[#FAFAFA] flex justify-between items-center flex-shrink-0">
          <div className="text-xs text-[#666]">
            Preview reflects current question pool • {questions.length}{" "}
            questions
          </div>
