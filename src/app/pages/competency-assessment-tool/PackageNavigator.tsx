...
export function PackageNavigator() {
  ...
  const documents = [
    ...
    {
      icon: <DocumentTextIcon className="w-8 h-8 text-gray-500" />,
    ...
    {
      icon: <PencilSquareIcon className="w-8 h-8 text-gray-900" />,
    ...
    {
      icon: <PresentationChartLineIcon className="w-8 h-8 text-orange-500" />,
    ...
    {
      icon: <ClipboardDocumentListIcon className="w-8 h-8 text-purple-500" />,
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "draft":
        return (
          <span className="px-2 py-1 rounded-full text-xs font-semibold bg-[#FFF3E0] text-[#F57C00]">
            Draft
          </span>
        );
      case "not-started":
        return (
          <span className="px-2 py-1 rounded-full text-xs font-semibold bg-[#F5F5F5] text-[#999]">
            Not Started
          </span>
        );
      case "locked":
  ...
  const packageFinalized = Object.values(currentStatus).every(
    (s) => s === "finalized",
  );
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-[#666]">
          View, edit, and download all 6 documents across 5 CATs packages (30
          total deliverables)
        </p>
      </div>
      ...
      <div className="bg-white border border-[#E0E0E0] rounded mb-6">
        <div className="flex border-b border-[#E0E0E0]">
          {(["A", "B", "C", "D", "E"] as const).map((pkg) => {
            ...
            const isComplete = Object.values(pkgStatus).every(
              (s) => s === "finalized",
            );
                  ...
                  <div className="text-xs">
                    {isComplete ? (
                      ...
                      <span className="text-[#999]">
                        <LockClosedIcon className="w-5 h-5 inline text-yellow-500" />{" "}
                        Locked
                      </span>
                    )}
                  </div>
        ...
        <div className="p-6">
                ...
                <div className="text-sm text-[#666] mb-3">
                  {agFinalized
                    ? `Assessor's Guide finalized - ${
                        packageFinalized
                          ? "All documents ready"
                          : "Remaining documents in progress"
                      }`
              ...
              <div>
                {packageFinalized && (
                  <button className="px-4 py-2 bg-[#2E7D32] text-white rounded text-sm font-medium hover:bg-[#1B5E20] transition-colors">
                    ...
                    Download Package {selectedPackage}
                  </button>
                )}
              </div>
            </div>
          ...
          <div className="space-y-3">
            {documents.map((doc) => {
              const status =
                currentStatus[doc.key as keyof typeof currentStatus];
                    ...
                    <div className="flex-1">
                      ...
                      {doc.key === "ag" && status === "draft" && (
                        <div className="text-xs text-[#1976D2] bg-[#E3F2FD] inline-block px-2 py-1 rounded">
                          <ExclamationTriangleIcon className="w-5 h-5 inline text-yellow-500" />{" "}
                          Finalize AG to unlock remaining 5 documents
                        </div>
                      )}
                    </div>
                  </div>
                  <div className="flex gap-2">
                    {!isLocked && status !== "not-started" && (
                      <button
                        ...
                        onClick={() => navigateToPage(doc.route)}
                      >
                        View
                      </button>
                    )}
          ...
          <div className="mt-6 p-4 bg-[#E3F2FD] border border-[#90CAF9] rounded">
            ...
            <ul className="text-xs text-[#1565C0] space-y-1 ml-5 list-disc">
              <li>
                <strong>Per-Package Documents:</strong> AG, SIC, Written Test,
                Rating Sheet contain unique content for Set {selectedPackage}
              </li>
              <li>
                <strong>Shared Documents:</strong> SAG and CARS are identical
                across all 5 packages (edit once, applies to all)
              </li>
              <li>
                AG must be finalized before the other 5 documents can be
                generated
              </li>
            </ul>
          </div>
      ...
      <div className="bg-white border border-[#E0E0E0] rounded p-5 mb-6">
        <div className="font-semibold text-sm text-[#333] mb-4">
          Overall Progress Across All Packages
        </div>
        <div className="grid grid-cols-5 gap-4">
          {(["A", "B", "C", "D", "E"] as const).map((pkg) => {
            const status = packageStatus[pkg];
            const totalDocs = 6;
            const finalizedDocs = Object.values(status).filter(
              (s) => s === "finalized",
            ).length;
            ...
            return (
              <div
                key={pkg}
                className="p-3 bg-[#FAFAFA] rounded border border-[#E0E0E0]"
              >
