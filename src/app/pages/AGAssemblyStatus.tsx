...
export function AGAssemblyStatus() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "draft":
        return (
          <span className="px-2 py-1 rounded text-xs font-semibold bg-[#FFF3E0] text-[#F57C00]">
            Draft
          </span>
        );
      case "not-started":
        return (
          <span className="px-2 py-1 rounded text-xs font-semibold bg-[#F5F5F5] text-[#999]">
            Not Started
          </span>
        );
      default:
  ...
  const finalizedCount = Object.values(agStatus).filter(
    (s) => s === "finalized",
  ).length;
    ...
    <div className="p-6 text-gray-800">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-[#666]">
          Assemble 5 {`Assessor's Guides`} (Sets A-E) from Phase 2 pools before
          generating remaining documents
        </p>
      </div>
      ...
      <div className="bg-white border border-[#E0E0E0] rounded p-5 mb-6">
        <div className="flex justify-between items-center mb-3">
          ...
          <span className="text-xs text-[#666]">
            {finalizedCount} of 5 {`Assessor's Guides`} finalized
          </span>
        </div>
        <div className="mb-2">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="font-semibold text-sm text-[#2E7D32] mb-2">
                {`All 5 Assessor's Guides Finalized!`}
              </div>
              <div className="text-sm text-[#1B5E20] mb-4">
                You can now proceed to Step 2: Generate the remaining 5
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm mb-3">
                {`Complete and finalize all 5 Assessor's Guides to unlock Step 2
                (remaining documents generation).`}
              </div>
                ...
                {(["A", "B", "C", "D", "E"] as const).map((set) => (
                  <div key={set}>
                    {agStatus[set] === "finalized" ? (
                      ...
                      <XMarkIcon className="w-5 h-5 inline text-red-500" />
                    )}{" "}
                    {`Assessor's Guide -`}
                    Set {set}
                  </div>
      ...
      <div className="mt-6 flex justify-between">
        ...
        <div className="flex gap-3">
          <button
            className="px-4 py-2 bg-white text-[#666] border border-[#E0E0E0] rounded text-sm font-medium hover:bg-[#F5F5F5] transition-colors"
            onClick={() => navigateToPage("distribution-settings")}
          >
