...
export function QuestioningToolEditor() {
  ...
  const knowledgeCount = questions.filter(
    (q) => q.testType === "Knowledge",
  ).length;
  const scenarioCount = questions.filter(
    (q) => q.testType === "Scenario",
  ).length;
  const totalQuestions = questions.length;

  const dimensionCounts = questions.reduce(
    (acc, q) => {
            ...
            <div className="p-5">
              <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-[#333]">
                        {knowledgeCount} (
                        {((knowledgeCount / totalQuestions) * 100).toFixed(0)}%)
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-[#2E7D32]">
                        {scenarioCount} (
                        {((scenarioCount / totalQuestions) * 100).toFixed(0)}%)
                      </span>
                    </div>
                  </div>
                </div>
                <div className="p-4 bg-[#F5F5F5] rounded">
                  ...
                  <div className="space-y-1.5">
                    {Object.entries(dimensionCounts).map(
                      ([dimension, count]) => (
                        <div
                          key={dimension}
                        ...
                        {editingId === q.id ? (
                          <div className="flex gap-1 justify-center">
                            <button
                              ...
                              onClick={handleSave}
                            >
                              <DocumentCheckIcon className="w-5 h-5 inline" />
                            </button>
                            <button
        ...
        <div className="bg-white border border-[#E0E0E0] rounded mb-6">
          ...
          <div className="p-6 space-y-6">
            <div className="p-4 bg-[#E3F2FD] border-l-4 border-[#1976D2]">
              <div className="font-semibold text-sm text-[#1976D2] mb-2">
            ...
            <div className="space-y-5">
              {questions.map((q) => (
                <div
                  key={q.id}
                  className={`border-l-4 ${q.testType === "Knowledge" ? "border-[#2E7D32]" : "border-[#F57C00]"} pl-4`}
                >
